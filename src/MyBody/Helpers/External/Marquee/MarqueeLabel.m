...
- (void)setupLabel {
    ...
    self.subLabel.tag = kSetupLabelSubtextFieldTag;
...
-(void)unpauseLabel
{
    if (self.isPaused) {
        ...
        self.subLabel.layer.speed = kLabelMarqueeSpeed;
...
- (CGFloat)solveTForY:(CGFloat)y_0 withEpsilon:(CGFloat)epsilon controlPoints:(NSArray *)controlPoints
{
    ...
    for (int i = 0; i < 15; i++) {
        ...
        if (fabs(df0) < 1e-6) {
            LogE(@"MarqueeLabel: Newton's Method failure, small/zero derivative!");
            break;
        }
        // Else recalculate t1
        t1 = t0 - f0/df0;
    ...
    LogI(@"MarqueeLabel: Failed to find t for Y input!");
    return t0;
}
