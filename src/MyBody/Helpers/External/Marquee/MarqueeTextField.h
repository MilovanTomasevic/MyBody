...
/**
 MarqueeTextField is a UILabel subclass adds a scrolling marquee effect when the text of a label instance outgrows the available width. Instances of `MarqueeTextField` can be configured
 for label scrolling direction/looping, speed/rate, and other options.
 */

@interface MarqueeTextField : MTTextField<CAAnimationDelegate>

////////////////////////////////////////////////////////////////////////////////
/// @name Creating MarqueeTextField
////////////////////////////////////////////////////////////////////////////////


/** Returns a newly initialized `MarqueeTextField` instance.
 ...
 @return An initialized `MarqueeTextField` object or nil if the object couldn't be created.
 */

- (instancetype)initWithFrame:(CGRect)frame;


/** Returns a newly initialized `MarqueeTextField` instance with the specified scroll rate and edge transparency fade length.
 ...
 @param fadeLength A length of transparency fade at the left and right edges of the `MarqueeTextField` instance's frame.
 @see fadeLength
 @return An initialized `MarqueeTextField` object or nil if the object couldn't be created.
 */
...
/** Returns a newly initialized `MarqueeTextField` instance with the specified scroll duration and edge transparency fade length.
...
/** A boolean property that sets whether the `MarqueeTextField` should behave like a normal UILabel.

 When set to `YES` the `MarqueeTextField` will behave like a normal UILabel, and  will not begin scrolling when the text is
 larger than the specified frame. The change takes effect immediately, removing any in-flight animation as well as any
 current edge fade. Note that the `MarqueeTextField` will respect the current values of the `lineBreakMode` and `textAlignment`
...
/** A boolean property that sets whether the `MarqueeTextField` should hold (prevent) label scrolling.

 When set to `YES`, the `MarqueeTextField` will not automatically scroll even its text is larger than the specified frame,
 although the specified edge fades will remain.

 To set the `MarqueeTextField` to act like a normal UILabel, use the `labelize` property.
...
/** A boolean property that sets whether the `MarqueeTextField` should only begin a scroll when tapped.

 If this property is set to `YES`, the `MarqueeTextField` will begin a scroll animation cycle only when tapped. The label will
...
/** Defines the direction and method in which the `MarqueeTextField` instance scrolls.

 `MarqueeTextField` supports four types of scrolling: `MLLeftRight`, `MLRightLeft`, `MLContinuous`, and `MLContinuousReverse`.
...
/** The length of transparency fade at the left and right edges of the `MarqueeTextField` instance's frame.

 This propery sets the size (in points) of the view edge transparency fades on the left and right edges of a `MarqueeTextField`. The
...
/** Convenience method to restart all `MarqueeTextField` instances that have the specified view controller in their next responder chain.

 This method sends a `NSNotification` to all `MarqueeTextField` instances with the specified view controller in their next responder chain.
 ...
 @warning View controllers that appear with animation (such as from underneath a modal-style controller) can cause some `MarqueeTextField` text
...
/** Labelizes all `MarqueeTextField` instances that have the specified view controller in their next responder chain.

 This method sends an `NSNotification` to all `MarqueeTextField` instances with the specified view controller in their next
 responder chain. The `labelize` property of these `MarqueeTextField` instances will be set to `YES`.

 @param controller The view controller for which all `MarqueeTextField` instances should be labelized.
 @see labelize
 */
...
/** De-Labelizes all `MarqueeTextField` instances that have the specified view controller in their next responder chain.

 This method sends an `NSNotification` to all `MarqueeTextField` instances with the specified view controller in their next
 responder chain. The `labelize` property of these `MarqueeTextField` instances will be set to `NO` .

 @param controller The view controller for which all `MarqueeTextField` instances should be de-labelized.
 @see labelize
 */
